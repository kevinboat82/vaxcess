...
import api from '../api';
import type { Worker } from '../pages/Dashboard';
...
export const EditWorkerModal: React.FC<EditWorkerModalProps> = ({ isOpen, worker, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await api.put(`/auth/workers/${worker.id}`, formData);
            onSuccess();
            onClose();
        } catch (err: any) {
            setError(err.response?.data?.error || 'Failed to update health worker.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
                    <div>
                        <h2 className="text-lg font-bold text-slate-800">Edit Worker Profile</h2>
                        <p className="text-sm text-slate-500">Update worker details or reset password.</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1 text-[13px] uppercase tracking-wider">
                            New Password <span className="text-slate-400 normal-case tracking-normal">(Leave blank to keep current)</span>
                        </label>
                        <div className="relative">
                    ...
                    <div className="mt-8 pt-4 border-t border-slate-100 flex gap-3">
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-70 flex items-center justify-center gap-2"
                        >
                            {isLoading ? (
